...
def calcubot_plot3d(SCRIPT_PATH,expression,god_mode,granted_words):
    try:
        message=''
        answer_max_lenght       = 4095
...
plt.show()
plt.close()
print('cubic_cat'+filepath+'cubic_dog')
"""
            script_uid = str(uuid.uuid4())
            eval_script_file = SCRIPT_PATH+'scripts/'+script_uid+'.txt'
            eval_cmd = 'try:'
            eval_cmd += "\n# multiline_eval(open('"+eval_script_file+"','r').read())"
            eval_cmd += '\n' + preamble + expression + postamble
            
            eval_cmd = eval_cmd.replace('\n','\n    ')
            eval_cmd += '\nexcept Exception as e:'
            eval_cmd += '\n    print("triangle_cat"+str(e)+"triangle_dog")\n'
            with open(eval_script_file, 'w') as f:
                f.write(eval_cmd)
            eval_cmd = "multiline_eval(open('"+eval_script_file+"','r').read())"
            filepath = str(secure_eval(SCRIPT_PATH, eval_cmd))
            #with open(SCRIPT_PATH+'scripts/'+script_uid+'.log', 'w') as f:
            #    f.write(filepath)
            pattern = re.compile(r'cubic_cat(.*)cubic_dog')
            match = pattern.search(filepath)
            if match:
                filepath = match.group(1)
            else:
                pattern = re.compile(r'triangle_cat(.*)triangle_dog')
                match = pattern.search(filepath)
                if match:
                    message = match.group(1)
                    filepath = ''
            secure_os.remove(eval_script_file)
        else:
            return message,''
...
def check(expression, answer_max_lenght, god_mode, granted_words):
    ...
    for sym in [expression[i] for i in range(len(expression))]:
        if granted_symbols.find(sym)==-1 and not sym=='\n':
